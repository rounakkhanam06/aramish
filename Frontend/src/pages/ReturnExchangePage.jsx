...
// Splits the admin-authored plain-text policy into blocks — a line like "1. RETURNS & REFUNDS"
    ...
    const fetchPolicy = async () => {
      try {
        ...
        if (res.ok && data.success) {
          setContent(data.returnExchange || '');
        }
      } catch (err) {
        console.error('Failed to load return & exchange policy:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="bg-surface min-h-[100dvh] font-sans animate-fade-in flex flex-col">
      ...
      <div className="bg-surface px-4 py-4 shadow-sm z-50 sticky top-0 flex items-center gap-3">
        ...
        <h1 className="text-[#02006c] text-[20px] font-black tracking-tight">Return & Exchange Policy</h1>
      </div>
      ...
      <div className="p-5 flex-grow space-y-6">
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-[#e8e8ff] rounded-2xl p-5 border border-indigo-100 shadow-sm relative overflow-hidden">
          <div className="relative z-10">
            <h2 className="text-[#02006c] font-black text-lg mb-1">Returns & Exchanges</h2>
            <p className="text-slate-600 text-xs font-medium max-w-[85%]">
              Everything you need to know about returning or exchanging an order.
            </p>
          </div>
          <RotateCcw className="absolute -bottom-4 -right-4 w-24 h-24 text-indigo-200/50 rotate-12" />
        </div>
